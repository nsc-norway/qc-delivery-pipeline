...
do
    ...
    if [ "$complete" = true ]; then
        ...
        if [ ! -f "$analysis/../../SapioInfo.yaml" ]; then
            echo "Extracting run information from Sapio into SapioInfo.yaml..." >> "$log_file"
            python3 sapio-run-extractor.py \
                "$analysis/../../RunInfo.xml" \
                --output-yaml-file "$analysis/../../SapioInfo.yaml" >> "$log_file" 2>&1
            echo "" >> "$log_file"
        fi
        echo "Running the nextflow pipeline..." >> "$log_file"
        pipeline-runner.sh "$analysis/../../SapioInfo.yaml" "$analysis" >> "$log_file" 2>&1
    fi
done
